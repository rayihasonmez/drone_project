...
while (1) {
    if (!skip_status_update) {
        ...
        json_object_object_add(jobj, "battery", json_object_new_int(100));
        ...
        skip_status_update = 0; // Sadece bir kez atla
    }
...
if (strcmp(status, "on_mission") == 0 && target_x >= 0 && target_y >= 0) {
        ...
        if (x == target_x && y == target_y) {
            ...
            // HEMEN ARDINDAN STATUS_UPDATE (idle) mesajı gönder
            struct json_object *idle_update = json_object_new_object();
            json_object_object_add(idle_update, "type", json_object_new_string("STATUS_UPDATE"));
            json_object_object_add(idle_update, "drone_id", json_object_new_string(argv[1]));
            json_object_object_add(idle_update, "timestamp", json_object_new_int(time(NULL)));
            struct json_object *loc2 = json_object_new_object();
            json_object_object_add(loc2, "x", json_object_new_int(x));
            json_object_object_add(loc2, "y", json_object_new_int(y));
            json_object_object_add(idle_update, "location", loc2);
            json_object_object_add(idle_update, "status", json_object_new_string("idle"));
            json_object_object_add(idle_update, "battery", json_object_new_int(100));
            json_object_object_add(idle_update, "speed", json_object_new_int(5));
            const char *idle_str = json_object_to_json_string(idle_update);
            send(sockfd, idle_str, strlen(idle_str), 0);
            json_object_put(idle_update);

            strcpy(status, "idle");
            target_x = target_y = -1;
            current_mission_id[0] = '\0';

            skip_status_update = 1; // Sonraki döngüde status_update atlanacak!
        }
    }
